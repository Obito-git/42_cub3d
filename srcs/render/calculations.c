...
void	set_side_dist_helper(t_data *data, t_ray *ray)
{
	...
	if (ray->side == 0 || ray->side == 1)
		ray->perpWallDist = ray->side_distance.x - ray->delta_distance.x;
	else
		ray->perpWallDist = ray->side_distance.y - ray->delta_distance.y;
}
...
void	set_texture_position(t_data *data, t_ray *ray)
{
    ...
    if (ray->side == 0 || ray->side == 1)
		ray->wall.x = data->player.pos.y + ray->perpWallDist * ray->ray_dir.y;
    else
		ray->wall.x = data->player.pos.x + ray->perpWallDist * ray->ray_dir.x;
